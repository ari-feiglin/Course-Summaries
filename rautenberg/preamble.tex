...
\def\@blist[#1]{%
    ...
    \def\@item[##1]{%
        ...
        \advance\leftskip by \dimexpr\wd0+\listindent\relax\leavevmode\kern-\wd0\box0%
    }
    \def\item{%
        \@ifnextchar[ \@item {\@item[#1]}%
    }%
    \bgroup%
}
...
\def\multlines#1{%
    \par\medskip%
    \hbox{\m@th\valign{%
        \hbox to\hsize{\kern.5cm$\displaystyle{##}$\hfil}\tabskip=1\jot&&\hbox to\hsize{\hfil$\displaystyle{##}$\kern.5cm}\tabskip=1\jot\crcr
        #1\crcr
    }}%
    \medskip%
}

\def\cosub{\mathbin{\vcenter{\offinterlineskip\ialign{\hfil$##$\hfil\cr\noalign{\kern3pt}\smash{\cdot}\cr\noalign{\kern-3pt}-\cr}}}}
\mathchardef\scrp="17D
\def\GN{{\it GN}}
\def\bparens#1{\mathchar"4628\relax#1\mathchar"5629\relax}
\def\#{\sharp}
\def\s{{\sf s}}
\def\tneg{\tilde{\neg}}
\def\tand{\tilde{\land}}
\def\tto{\tilde{\to}}
\def\teq{\tilde{\eq}}
\def\tforall{\tilde{\forall}}
\def\texists{\tilde{\exists}}
\def\tS{\tilde{\tt S}}
\def\tplus{\tilde{+}}
\def\tcdot{\mathbin{\tilde{\cdot}}}
\def\proof{{\it proof}}
\def\bew{{\it bew}}
\def\bwb{{\it bwb}}
