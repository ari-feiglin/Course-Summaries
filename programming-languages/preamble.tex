...
\def\centergrnd#1{{
    \setbox0=\hbox{#1}%
    \lower.5\dimexpr\ht0-\dp0\relax\box0\relax%
}}
...
\def\@@@@stree#1[#2]#3{%
    \ifx\_nul#1%
        \quitvmode\kern-2\dimexpr\streebuf\relax\relax%
    \else
        \_afterfi{%
            \ifx\noc#3%
                \hbox{$#1$\quad #2}%
            \else%
                \@stree{#1}[#2]{#3}%
            \fi%
            \kern3\dimexpr\streebuf\relax\hfil\@@@stree%
        }%
    \fi%
}

\def\@@@stree#1{\_ifnextchar[ {\@@@@stree{#1}}{\@@@@stree{#1}[]}}
...
\def\@@stree#1[#2]#3{%
    \vbox{
        ...
        \ifx\empty\_regA
            \hbox{$#1$}%
        \else%
            \tabskip=\streebuf\ialign{##&##\hfil\tabskip=\streebuf\cr
                \hfil\kern\streebuf\@@@stree#3\_nul\_nul\cr\noalign{\kern3pt}
                \leaders\hrule\hfil&\centergrnd{#2}\cr\noalign{\kern3pt}
                \hfil$#1$\hfil\cr%
            }
        \fi%
    }
...
\def\syntaxtree#1{\hbox{$\vcenter{\offinterlineskip\@stree#1}$}}
...
\def\colcoleqq{\mathrel{{\mathop:}{\mathop:}\!\!=}}

\def\hdi{\buildrel\partial I\over\sim}
\def\htopic#1{\buildrel #1\over\sim}

\def\expansion#1{%
    \par\centerline{\vbox{\ialign{\hfil$##{}$\tabskip=.25cm&\tt##\hfil\tabskip=.5cm&##\hfil\tabskip=0pt\crcr #1\crcr}}}%
}

\def\eqdef#1{
    \centerline{\vbox{\ialign{\hfil$\lcalc{##}={}$\tabskip=\z@&\lcalc{##}\hfil\tabskip=\z@\crcr
    #1\crcr
    }}}
}

\let\olam=\lambda
\def\lambda{\relax\ifmmode\olam\else$\olam$\fi}
\let\ouline=\underline
\def\underline#1{\relax\ifmmode\ouline{#1}\else$\ouline{\hbox{#1}}$\fi}
\let\oldto=\to
\def\to{\relax\ifmmode\oldto\else$\oldto$\fi}
\let\oldmapsto=\mapsto
\def\mapsto{\relax\ifmmode\oldmapsto\else$\oldmapsto$\fi}
\let\oldtt=\tt
\def\tt{\oldtt\sfcode`.=0\relax}

\def\cn#1{\hbox{${\tt c}_{#1}$}}
...
\def\iszro{{\tt iszro}}
\def\Bool{{\sf Bool}}
\def\scr#1{\hbox{$\m@th{}_{\hbox{\setscale{7pt}#1}}$}}

\def\lcalc#1{\hbox{\tt\frenchspacing#1}}

\def\ifelse#1#2#3{{\tt if}\;#1\;{\tt then}\;#2\;{\tt else}\;#3}
\def\whiledo#1#2{{\tt while}\;#1\;{\tt do}\;#2}
\def\[{[\![}\def\]{]\!]}
\def\fun{{\sf fun}\ }

\appendarraymanyvalued{colors}{rgb{0 1 .3}}{{type}{list}{let}{rec}{fun}{match}{with}{then}{if}{else}{and}{in}{of}}
\appendarraymanyvalued{colors}{rgb{0 1 .3}}{*:|=@}
\appendarray{initcolors}{{"}{"}{rgb{.8 .8 .3}}}
\def\syntax@buf{.15cm}
