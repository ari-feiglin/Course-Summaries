...
\smallskip
\centerline{\tt times = \lambda n.\lambda m.\lambda s.\lambda z.m (\plus\ n) \cn0}

so that \lcalc{times n m s z} will apply \lcalc{\plus n} $m$ times to \cn0, resulting in $n+n+\cdots+n+0=n\cdot m$.
In a similar vein, we can define \lcalc{pow = \lambda n.\lambda m.\lambda s.\lambda z.m (times\ n) \cn1}, so that \lcalc{pow \cn n \cn m} is equal to \cn{n^m}.
