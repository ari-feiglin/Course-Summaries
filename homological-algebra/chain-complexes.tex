...
Elements of $\Ch(\Ch(\A))$ are families $\set{C_{p,q}}_{p,q\in\bZ}$ with horizontal and vertical differentials $d^h\colon C_{p,q}\to C_{p-1,q}$ and $d^v\colon C_{p,q}\to C_{p,q-1}$ such that
$$ d^h\circ d^h = d^v\circ d^v = 0,\qquad d^h\circ d^v = d^v\circ d^h $$
Indeed, a chain complex of chain complexes consist of a family of chain complexes $\set{\set{C_{p,q},d^h}_p}_q$ and differentials $d^v\colon C_{\bul,q}\to C_{\bul,q-1}$.
The first two equalities above say that these are differentials, the last says that $d^v$ is a chain map.

A map of bicomplexes $f\colon C_{\bul\bul}\to D_{\bul\bul}$ is then a family of maps $f_{p,q}\colon C_{p,q}\to D_{p,q}$ such that $f_{p,\bul},f_{\bul,q}$ are chain maps for all $p,q$.

For reasons which will become clearer later, we call such a construction a {\emphcolor positive bicomplex}.
We define a (signed) bicomplex as follows:
...
This ``sign trick'' allows us to identify the category of bicomplexes with $\Ch(\Ch(\A))$ (i.e.\ this defines an isomorphism of categories).
...
The reason for using (signed) bicomplexes is because they simplify the construction of the total complexes.
If we were to use a positive bicomplex, then the total complex would have differential $d^h+(-1)^pd^v$.

Note that both total complex constructions form functors $\Ch(\Ch(\A))\to\Ch(\A)$.
Indeed, if $f_{p,q}\colon C_{p,q}\to D_{p,q}$ is a map of bicomplexes, then it defines a chain map between total complexes in a natural way.
Map $(a_{p,q})_{p+q=n}\in\tot(C)_n$ to $(f_{p,q}a_{p,q})_{p+q=n}\in\tot(D)_n$.
This is indeed a chain map:
$$ f(d^ha_{pq}+(-1)^pd^va_{pq}) = (d^hfa_{pq}+(-1)^pd^vfa_{pq}) = (d^h+(-1)^pd^v)(fa_{pq}) $$
...
These are the {\emphcolor brutal truncations} of a chain complex $C$: $(\sigma_{<n}C)_i=C_i$ when $i<n$ and $0$ otherwise.
And then $\sigma_{\geq n}C=C/(\sigma_{<n}C)$.
Note that
$$ H_n(\sigma_{\geq n}C) = C_n/B_n $$
...
These are chain maps: $d(0,c)=(0,dc)$ and $d(b,c)=(-db,dc-fb)\mapsto -db$ (recall the differential of $B[-1]$ is $-d_B$ by the sign convention).
