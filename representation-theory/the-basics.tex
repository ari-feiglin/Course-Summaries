...
\subsection{The classical Fourier transform}

Viewing $\bF[G]$ as the function space $\St(G,\bF)$, we can view the Fourier transform as
$$ \F\colon\St(G,\bF) \to \prod_i\endo_{D_{E_i}}(E_i) $$
Recall that the algebra $\St(G,\bF)$ has multiplication given by convolution: writing $f=\sum_{g\in G}f(g)\delta_g$ we have
$$ f_1*f_2 = \sum_{g\in G}f_1(g)\delta_g \sum_{h\in G}f_2(h)\delta_h = \sum_{g,h\in G}f_1(g)f_2(h)\delta_{gh} = \sum_{g\in G}\parens{\sum_{h\in G}f_1(h)f_2(h^{-1}g)}\delta_g $$
i.e.\ $(f_1*f_2)(g)=\sum_{h\in G}f_1(h)f_2(h^{-1}g)$ which is precisely the definition of a convolution.

On the other hand, the product space (the codomain of $\F$) has its multiplication defined by (pointwise) function composition.
That is, $\F(f_1*f_2)=\F(f_1)\circ\F(f_2)$.

In the case of Abelian $G$, the Fourier transform becomes
$$ \F\colon\St(G,\bF) \to \St(\ch_\bF(G),\bF) $$
And function composition in the codomain becomes pointwise {\it multiplication}.
This is because the functions in the product space are scalar multiplications, and composition is equivalent to multiplication.
So we can write $\F(f_1*f_2)=\F(f_1)\cdot\F(f_2)$, which is reminiscent of the classical Fourier transform.

Recall that the classical discrete Fourier transform takes a sequence of complex numbers $x_0,\dots,x_{N-1}$ and transforms them into
a new sequence of complex numbers $X_0,\dots,X_{N-1}$ defined by
$$ X_k = \sum_{n=0}^{N-1}x_n\omega_N^{-kn} $$
where $\omega_N$ is the primitive root of unity $\exp(2\pi i/N)$.

Now, we can view series of complex numbers as functions $(x_i)_i\colon\bZ/N\bZ\to\bC$.
So we can ask ourselves, what if we take the Abelian Fourier transform of this function?
We see that $(x_i)_i=\sum_{i=0}^{N-1}x_i\delta_i$, and so
$$ \F((x_i)_i) = \sum_{n=0}^{N-1}x_n\sum_{\chi\in\ch_\bC(\bZ/N\bZ)}\chi(n)\delta_\chi $$

Now, we ask ourselves, what are the characters of $\bZ/N\bZ$ over $\bC$?
Since $\bZ/N\bZ$ is cyclic, every group morphism $\bZ/N\bZ\to\bC^\times$ is determined by its image on $1$.
And since $1$ has order $N$ in $\bZ/N\bZ$, $\chi(1)^N=1$ in $\bC$, so $\chi(1)$ is an $N$th root of unity, that is $\chi(1)=\omega_N^k$ for some $0\leq k<N$.
We claim that $\chi\mapsto\chi(1)$ gives a bijection between characters and $\mu_N(\bC)$ ($N$th roots of unity).
It is simple to see that every $\omega_N^k$ gives rise to a character.
Furthermore, $\mu_N(\bC)\cong\bZ/N\bZ$ (generated by $\omega_N$), so we can view the Fourier transform as a function
$$ \F\colon\St(\bZ/N\bZ,\bC)\to\St(\bZ/N\bZ,\bC) $$

Now we return to our computation.
We take the bijection between $\bZ/N\bZ$ and $\ch_\bC(\bZ/N\bZ)$ which maps $[n]$ to $\chi_n(1)=\omega_N^{-n}$.
Note that the exact bijection we choose will not affect the outcome; it only affects the order of the resulting series.
Since $\chi_n$ is a character, $\chi_n(k)=\chi_n(1)^k=\omega_N^{-kn}$.
Thus we get
$$ \F((x_i)_i) = \sum_{n=0}^{N-1}x_n\sum_{k=0}^{N-1}\omega_N^{-kn}\delta_k = \sum_{k=0}^{N-1}\parens{\sum_{n=0}^{N-1}x_n\omega_N^{-kn}}\delta_k $$
i.e.\ $\F((x_i)_i)$ is the series $(X_k)_k$ defined by
$$ X_k = \sum_{n=0}^{N-1}x_n\omega_N^{-kn} $$
which is precisely the discrete Fourier transform.

Now, we want to find the inverse discrete Fourier transform, namely $x_n$ in terms of $X_k$.
To do this we find the inverse of $\delta_{\chi_k}$ (where $\chi_k(1)=\omega_N^{-k}$).
This is $e_k=e_{\chi_k}$:
$$ e_k = \frac1N\sum_{n=0}^{N-1}\chi_k(n)^{-1}\delta_n = \frac1N\sum_{n=0}^{N-1}\omega_N^{nk}\delta_n $$
Now, $(X_k)_k=\sum_{k=0}^{N-1}X_k\delta_{\chi_k}$, and so the inverse discrete Fourier transform of that is
$$ (x_n)_n = \sum_{k=0}^{N-1}X_ke_k = \frac1N\sum_{k=0}^{N-1}X_k\sum_{n=0}^{N-1}\omega_N^{nk}\delta_n $$
Moving things around, we get
$$ x_n = \frac1N\sum_{k=0}^{N-1}X_k\omega_N^{nk} $$
