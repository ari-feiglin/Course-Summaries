...
\bdefn
    ...
    A \emphasize{topological manifold of dimension $n$} (or an \emphasize{$n$-manifold}) is a topological space $M$ such that
    \benum
        ...
        \item $M$ is \emphasize{locally Euclidean} of dimension $n$: every point is contained in a neighborhood homeomorphic to an open subset of $\bR^n$.
    \eenum
...
\bdefn
    ...
    A \emphasize{coordinate chart} on $M$ is a pair $(U,\phi)$ with $U\subseteq M$ and $\phi\colon U\to\bR^n$ an embedding (equivalently $\phi\colon U\to\phi(U)\subseteq\bR^n$
    a homeomorphism).
    $U$ is the \emphasize{coordinate domain} or \emphasize{coordinate neighborhood} of the chart, $\phi$ is the \emphasize{(local) coordinate map} the coordinates of $\phi$
    (i.e.\ the composition with the projections $\bR^n\to\bR$) are called the \emphasize{local coordinates} on $U$.

    By the definition of a manifold, every point in $M$ is contained in the domain of some chart.
    A chart $(U,\phi)$ is \emphasize{centered} at $p\in U$ if $\phi(u)=0$.
...
\bexam
    ...
    The \emphasize{$n$-dimensional real projective space}, denoted $\RP^n$ is the quotient of $\bR^{n+1}-0$ by equating scalar multiples of vectors.
...
Recall that the \emphasize{$n$-torus} is the $n$-fold product $T^n=S^1\times\cdots\times S^1$, and by this proposition and the above example is an $n$-manifold.
...
\bdefn
    ...
    \benum
        \item \emphasize{connected} if it cannot be written as the disjoint union of two non-empty open subsets (equivalently it has no non-trivial clopen sets);
        \item \emphasize{path connected} if every two points in $X$ can be connected by a path (a continuous $\gamma\colon[0,1]\to X$ whose endpoints are the given points);
        \item \emphasize{locally path connected} if $X$ has a basis of path-connected open subsets.
    \eenum

    Furthermore, $X$'s \emphasize{connected components} is the set of its maximally connected subsets.
    Its \emphasize{path components} is the set of its maximally path connected subsets.
...
\bdefn
    ...
    A topological space $X$ is \emphasize{locally compact} if every point has a \emphasize{compact neighborhood}.
    That is to say, for every $x\in X$ there is an open $U$ and compact $K$ such that $x\in U\subseteq K$.

    A \emphasize{precompact} set in $X$ is a subset $Y\subseteq X$ whose closure is compact.
...
\bdefn
    ...
    Then a collection $\c X$ of subsets of $X$ is \emphasize{locally finite} if every $x\in X$ has a neighborhood that intersects at most finitely many subsets in $\c X$.
    Given a cover $\c U$ of $X$, a \emphasize{refinement} of $\c U$ is another cover $\c V$ such that every set in $\c V$ is contained in some set in $\c U$.
    $X$ is \emphasize{paracompact} if every open cover of $X$ admits an open locally finite refinement.
...
Such functions are also called \emphasize{smooth}.
A smooth function with a smooth inverse is called a \emphasize{diffeomorphism}.

\bdefn

    Let $M$ be an $n$-manifold.
    Let $(U,\phi),(V,\psi)$ be two non-disjoint charts.
    Consider then the map $\psi\circ\phi^{-1}\colon\phi(U\cap V)\to\psi(U\cap V)$, this is called the \emphasize{transition map} from $\phi$ to $\psi$.
    As the composition of homeomorphisms, it itself is a homeomorphism.
    Two charts $(U,\phi),(V,\psi)$ are \emphasize{smoothly compatible} if they are either disjoint or their transition map is a diffeomorphism.

    An \emphasize{atlas} for $M$ is a collection of charts whose domains cover $M$.
    A \emphasize{smooth atlas} for $M$ is an atlas whose charts are all smoothly compatible.
...
\bdefn
    ...
    Let $M$ be a manifold, then a \emphasize{smooth structure} on $M$ is a maximal smooth atlas (i.e.\ one not contained in any other atlas).
    A \emphasize{smooth manifold} is a pair $(M,\A)$ where $\A$ is a smooth structure on a topological manifold $M$.
...
\bdefn
    ...
    A map $f\colon M\to N$ is \emphasize{smooth} if for every chart $p\in M$ there are charts $(U,\phi)\in\A,(V,\psi)\in\B$ with $p\in U$ and $f(U)\subseteq V$ such that
    the composite $\psi\circ f\circ\phi^{-1}\colon\phi(U)\to\psi(V)$ is smooth.
    A bijection $f$ is a \emphasize{diffeomorphism} if it is smooth and has a smooth inverse.
...
\bexam
    ...
    This is called the \emphasize{standard smooth structure}.
...
\bexam
    ...
    This is called an \emphasize{open submanifold} of $M$.
...
\bexam
    ...
    Using the abovbe lemma, we can turn this into a smooth manifold called the \emphasize{Grassmanian manifold}.
...
Define the \emphasize{closed $n$-dimensional upper half-space} $\bH^n\subseteq\bR^n$ as the set of all points whose $n$th coordinate is
nonnegative:
...
\bdefn
    ...
    An $n$-dimensional \emphasize{(topological) manifold with boundary} is a second-countable Hausdorff space $M$ such that every
    point in $M$ lies in a neighborhood homeomorphic to an open subset of $\bH^n$.

    A \emphasize{chart} for $M$ is a pair $(U,\phi)$ for $U\subseteq M$ open and $\phi\colon U\to\phi(U)$ a homeomorphism to an open subset $\phi(U)$
    of $\bH^n$.
    Call a chart a \emphasize{boundary chart} if $\phi(U)\cap\partial\bH^n\neq\emptyset$, and an \emphasize{interior chart} otherwise
    (if its image lies in the interior of $\bH^n$).
    An \emphasize{atlas} for $M$ is a collection of charts whose domains cover $M$.

    Call $p\in M$ a \emphasize{boundary point} if it lies in the domain of some boundary chart $(U,\phi)$ such that $\phi(p)\in\partial\bH^n$, and
    an \emphasize{interior point} if it lies in the domain of some interior chart.
...
called \emphasize{half-balls}.
If $(U,\phi)$ is a chart with $\phi(U)$ a half-ball, call $U$ a \emphasize{coordinate half-ball}.
...
Recall that a function $f\colon A\to\bR^m$ for $A\subseteq\bR^n$ arbitrary is \emphasize{smooth} if $A$ has an open neighborhood $U$ such that
...
\bdefn
    ...
    Then an \emphasize{atlas} for $M$ is a set of charts $(U_i,\phi_i)$ whose domain covers $M$.
    A \emphasize{smooth atlas} for $M$ is an atlas for which all the transition maps $\phi_j\circ\phi_i^{-1}$ are smooth in the sense explained
    above.
    A \emphasize{smooth structure} for $M$ is a maximally smooth atlas.
    A \emphasize{smooth manifold with boundary} is a manifold $M$ along with a choice of a smooth structure $\A$.
...
\bdefn
    ...
    A map $F\colon M\to N$ is \emphasize{smooth} if for every point $p\in M$, there are smooth charts $(U,\phi)\in M$ and $(V,\psi)\in N$
    such that $p\in U$ and $f(U)\subseteq V$ and $\psi\circ f\circ\phi^{-1}\circ\phi(U)\to\psi(V)$ is smooth.

    A \emphasize{diffeomorphism} is a smooth map with a smooth inverse.
