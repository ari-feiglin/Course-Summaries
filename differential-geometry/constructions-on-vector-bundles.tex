...
\bdefn
    ...
    If $V\in\X(M)$ is a smooth vector field, and $A$ a covariant tensor field on $M$, define its {\emphcolor Lie derivative} with respect to $V$
    by
...
\bdefn
    ...
    If $A$ is a covariant tensor field on $M$, and $\theta$ is a flow on $M$, then $A$ is {\emphcolor invariant under $\theta$} if for all
...
\bdefn
    ...
    Then a single-variable covariant {\emphcolor $C^\infty$ functor} is a functor $\F\colon\Vec\to\Vec$ such that the maps $\hom_\bR(V,W)\to\hom_\bR(\F(V),\F(W))$ are smooth
...
\blemm
    ...
    \benum
        ...
        \item $P$ is {\emphcolor reflexive}: $P^{**}\cong P$.
