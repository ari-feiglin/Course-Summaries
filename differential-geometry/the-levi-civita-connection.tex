...
\bproof
    ...
    So
    $$ \Gamma^k_{ij}g_{k\ell} = \frac12\parens{\partial_ig_{j\ell} + \partial_jg_{i\ell} - g_\ell g_{ij}} . $$
...
\bprop[title={Naturality of the Levi-Civita connection}]

    The Levi-Civita connection is natural.
    Suppose $\phi\colon(M,g)\to(\tilde M,\tilde g)$ is an isometry of Riemannian manifolds, and let $\tilde\nabla$ be the
    Levi-Civita connection on $\tilde M$.
...
\bnote
    ...
    $$ \hess f(Y,X) = \iprod{\nabla_X\grad f,Y} , $$
    where $\grad f=(df)^\sharp$, i.e.\ it is the unique vector field where $\iprod{\grad f,Y}=df(Y)=Yf$.
    Indeed, the right-hand side is
    $$ \iprod{\nabla_X\grad f,Y} = X\iprod{\grad f,Y} - \iprod{\grad f,\nabla_XY} = X(Yf) - (\nabla_XY)f . $$

    It is easy to see that in the case of $M=\bR^n$,
    $$ \hess f(\partial_i,\partial_j) = f_{ij} = \pdvof{^2f}{x_i\partial x_j} . $$
...
\bdefn
    ...
    Then $\exp_p(B_\epsilon(0))$ is a normal neighborhood of $p$, called a {\emphcolor geodesic ball}.
    If $\cl B_\epsilon(0)\subseteq V$, then $\exp_p(S_\epsilon(0))$, where $S_\epsilon(0)=\partial B_\epsilon(0)$,
    is called the {\emphcolor geodesic sphere} around $p$.
