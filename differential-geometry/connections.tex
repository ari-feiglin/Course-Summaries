...
\bdefn
    ...
    Let $\nabla$ be a connection on $M$, and $F\colon A\to M$ a smooth
    map from a submanifold $A\subseteq\bR^n$ to $M$.
    For a vector field $X\in\X(F)$, we define its {\emphcolor covariant derivatives} to be the vector fields along $F$
    given by
    $$ \frac{DX}{\partial t^i} = \nabla^F_{\ipdv{t^i}}X,\qquad i=1,\dots,n . $$

    In particular, for $n=1$, $F=\gamma$ is a curve (when $A$ is connected), and the covariant derivative along $\gamma$
    is denoted by $\frac{DX}{dt}$ or $D_tX$.

\edefn

\bthrm[title=Existence and uniqueness of parallel transport]

    Let $\gamma\colon[a,b]\to M$ be a curve, $t_0\in[a,b]$, and $v\in T_{\gamma(t_0)}M$.
    Then there is a unique vector field $X\in\X(\gamma)$ along $\gamma$ such that $X(t_0)=v$ and $D_tX=0$.
...
\bproof
    ...
    Otherwise can cover $[a,b]$ with subintervals which $\gamma$ maps into coordinate domains of $M$, and which intersect
    locally finitely.
