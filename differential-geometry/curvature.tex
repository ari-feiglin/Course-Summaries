...
\bdefn
    ...
    to a tensor $\sff^\flat\colon\X(M)\times\X^*(M)\to\Gamma(TM^\perp)$, which we can then take the trace of.

    Let $H=\tr\sff^\flat=\tr_g\sff$ be this trace, it is a section in $\Gamma(TM^\perp)$ called the {\emphcolor mean curvature} of $M$.
    Explicitly for an orthonormal basis of tangent vectors $e_1,\dots,e_n\in T_pM$,
