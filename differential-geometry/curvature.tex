...
\subsection{Curvature on vector bundles; the second fundamental form}
...
\bexam
    ...
    A submanifold $M\subseteq\bar M$ is {\emphcolor totally geodesic} if for every $p\in M$ and $v\in T_pM$, the restricted exponential
    of $\bar M$ lies in $M$: $\exp_p(tv)\in M$ for all $t$.

    If $M$ is totally geodesic, then its second fundamental form is zero $\sff=0$.
    In particular it has zero mean curvature.

    If $\phi$ is an order-two isometry of $\bar M$, and $M=\fix(\phi)$ is the set of fixed points (which we know is a submanifold, from homework).
