...
\subsection{Curvature on vector bundles}
...
\bnote
    ...
    Some authors have the convention that curvature is given by the opposite sign:
    $$ R(X,Y)s = \nabla_Y\nabla_Xs - \nabla_X\nabla_Ys + \nabla_{[X,Y]}s . $$
    In this case Gauss's formula gives
...
Consider the special case that the ambient manifold has dimension one greater than the one of interest: $\dim\bar M=\dim M+1$.
...
Furthermore $b(v,v)b(w,v)-b(v,w)^2$ is the determinant of $\beta|_\sigma$, so
$$ \bar K_p(\sigma) = K_p(\sigma) - \det\beta|_\sigma . $$

Further suppose that $\bar M$ has zero sectional curvature: $\bar K=0$.
Then
$$ \beta(x) = -\sff^\perp(X)\eta = -\bar\nabla_X\eta + \nabla^\perp_X\eta . $$
The second term is zero, since the connection is metric so
$$ 0 = X\iprod{\eta,\eta} = 2\iprod{\nabla^\perp_X\eta,\eta} , $$
and $\eta$ spans the normal bundle so $\nabla^\perp_X\eta=0$.
And from before we know
$$ K_p(\sigma) = \det\beta|_\sigma . $$

Note that $\eta$ is called the {\emphcolor Gauss map}, and $d\eta=\beta$.

Take for example $M=S^n_r\subseteq\bR^{n+1}$ the sphere of radius $r$.
Then $\eta\in\Gamma(\nu_M)$ is given by mapping $p\in S^n_r$ to $\frac1rp\in T_p\bR^{n+1}$, and this is orthogonal to the tangent space of the
sphere.
Then $\eta$ has differential $1/r$, which means that the $\beta(X)=1/rX$ and thus its determinant is $1/r^2$.
So the sectional curvature of the sphere is $1/r^2$.
