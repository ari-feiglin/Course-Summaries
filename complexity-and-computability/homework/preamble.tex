...
\newmathtype{parens}({})
\newmathtype{bracks}[{}]
\newmathtype{iprod}\langle|\rangle
...
\def\slfrac#1#2{\left.{}^{#1}\mkern-4mu\middle/\mkern-1mu{}_{#2}\right.}
\let\ds=\displaystyle
...
\def\benum{\bgroup%
    ...
    \def\item{%
        ...
        %\leavevmode\kern-\enumindent%
        \quitvmode\llap{\enumstyle{\enumcount} }%\kern\enumindent%
    }%
    \def\btext{%
...
\def\algorithm{%
    \line@num=1\relax%
    \ialign to \hsize\bgroup
        \kern\leftskip\hfil##\tabskip=.25cm&\kern\alg@indent##\hfil\tabskip=\z@\crcr
}

\def\ealgorithm{\crcr\egroup}
\def\State{\crcr{\sevenrm\the\line@num.}\global\advance\line@num by 1\relax&}
...
\def\EndChoose{\endblock{\textbf{endchoose}}}
\def\Choice{%
