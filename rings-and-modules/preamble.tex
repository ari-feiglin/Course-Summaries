...
\def\@fonttable{%
	...
	\halign{\ftable@strut\vrule\hskip.5em\hss##\hss\hskip.5em\vrule&&\hskip.5em\hss##\hss\hskip.5em\vrule\cr
		\multispan{17}\hss{\fontuse cmcsc14 \the\toks0\ Fonttable}\vrule width 0pt depth 5pt\hss\cr\noalign{\hrule}%
...
\def\@sqrt#1{%
	\ifnum #1 > 0\relax%
		\expandafter\math@dimen@result\expandafter=\the\dimexpr%
			(\math@dimen@result + (1pt * \math@dimen@input / \math@dimen@result)) / 2 \relax%
		\expandafter\@sqrt\expandafter{\the\numexpr#1-1}%
	\fi%
}

\def\sqrt@into#1{%
	\math@dimen@input=#1pt \math@dimen@result=1pt%
	\@sqrt{10}%
}

% \calcsqrt{n} -> the square root of n
\def\calcsqrt#1{%
	\sqrt@into{#1}\@strip@math@dimen%
}

\def\@get@decimal@part#1.#2#3\nul{#2}
\def\@get@int@part#1.#2\nul{#1}

\def\get@decimal@part#1{\expandafter\@get@decimal@part#1\relax\nul}
\def\get@int@part#1{\expandafter\@get@int@part#1\nul}

\newdimen\math@regA

\def\@round{%
	\math@dimen@result=\get@decimal@part{\the\math@dimen@input}pt%
	\math@regA=\get@int@part{\the\math@dimen@input}pt%
	\ifdim\math@dimen@result < 5pt%
		\math@dimen@result=\math@regA%
	\else%
		\math@dimen@result=\math@regA%
		\advance\math@dimen@result by 1pt%
	\fi%
}

% \round{f} -> closest integer to f
\def\round#1{
	\math@dimen@input=#1pt%
	\@round%
	\get@int@part{\the\math@dimen@result}%
}

% \isqty{n} -> round(sqrt(n))
\def\isqrt#1{%
	\sqrt@into{#1}%
	\expandafter\round\expandafter{\@strip@math@dimen}%
}
...
\def\newmathtype#1#2#3#4{%
    \ifx$#3$
        \expandafter\def\csname #1\endcsname##1{\left#2 ##1 \right#4}%
    \else%
...
\newmathtype{abs}\lvert{}\rvert
\newmathtype{floor}\lfloor{}\rfloor
\newmathtype{ceil}\lceil{}\rceil
\newmathtype{norm}\|{}\|
\newmathtype{cycle}<\vert>

\def\@newfunc@#1#2#3#4#5{%
    \expandafter\def\csname #1of@master\endcsname[##1]##2[##3]{\csname #1\endcsname_{##1}\left#3##2\;\middle#4\;##3\right#5}%
    \expandafter\def\csname #1of@\endcsname[##1]##2{\@ifnextchar[ {\csname #1of@master\endcsname[##1]{##2}}{\csname #1\endcsname_{##1}\left#3##2\right#5}}%
    \expandafter\def\csname #1of@@\endcsname##1{\@ifnextchar[ {\csname #1of@master\endcsname[]{##1}}{\csname #1\endcsname\left#3##1\right#5}}%
...
\def\slfrac#1#2{\mathchoice{\@slfrac\textstyle{#1}{#2}}{\@slfrac\scriptstyle{#1}{#2}}{\@slfrac\scriptscriptstyle{#1}{#2}}{\@slfrac\scriptscriptstyle{#1}{#2}}}
\def\@slfrac#1#2#3{\left.{}^{#1#2}\mkern-2mu\middle/\mkern-3mu{}_{#1#3}\right.}
\let\ds=\displaystyle
...
\newcount\enumdepth
\def\enumindent{.75cm}
\expandafter\def\csname enumstyle1\endcsname#1{$(\the#1)$}
\expandafter\def\csname enumstyle2\endcsname#1{$({\rm\romannumeral#1\relax})$}
\def\benum{\bgroup\par%
    \enumcount=0\relax%
    \advance\enumdepth by 1\relax%
    \def\item{%
        \par\egroup\advance\enumcount by 1\bgroup\medskip%
        \advance\leftskip by \enumindent\relax\leavevmode\kern-\enumindent%
        \rlap{\csname enumstyle\the\enumdepth\endcsname{\enumcount}}\kern\enumindent%
    }%
    \bgroup%
}
...
\def\symdif{\mathop{\vcenter{\hbox{\scalebox{.6}{$\triangle$}}}}}
